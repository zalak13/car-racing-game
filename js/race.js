...
var userActions = (function () {
    ...
    var colorTrucks = ["Cyan", "Blue", "Green", "Purple", "Red", "Whitesmoke"];
    ...
    function placeBet(trucksBetAmountArray) {
        ...
        for (var i = 0; i < noOfTrucks; i++) {
            document.getElementById('amt' + i).innerHTML = trucksBetAmountArray[i]["betAmountOnTruckNumber"];
        }
    }
    ...
    function resetRaceSetUp(playerName, numOfTrucks, fundAmount) {
        ...
        if (numOfTrucks != "" || numOfTrucks != " " || numOfTrucks != "Choose number of truckers" || numOfTrucks == undefined) {
        ...
        if (defaultTruck == undefined || defaultTruck == "Choose number of truckers") {
            defaultTruck = 4;
        }
    ...
    function modalSetUp() {
        ...
        $('.truckForm').remove();
        $('.modal-body').append(
            '<form class="form-horizontal truckForm" id ="placeBetForm">' +
            '</form>'
        )
        for (var i = 0; i < defaultTruck; i++) {
            ...
            $('.truckForm').append(
                ...
                '</div>' +
                '</div>');
            document.getElementsByClassName('truckLabel')[i].setAttribute('id', TruckerNameId);
            document.getElementsByTagName('input')[i].setAttribute('id', TruckerNumId);
            document.getElementsByTagName('input')[i].setAttribute('name', TruckerNumId);
            var colorTruck = "truck" + i;

            var trucNo = i + 1;
            $("#TruckerName" + i).text("Trucker " + trucNo + " (" + colorTrucks[i] + ")");
    ...
    function endRace(winnerTrucker) {
        ...
        var betamount = trucksBetAmountArray[indexofTruck]["betAmountOnTruckNumber"];
        if (betamount > 0) {
            var currentfunds = document.getElementById("initialFund");
            currentfunds.innerHTML = parseInt(getPlayerAmount()) + (2 * parseInt(betamount));
            var message = document.getElementById("msgDiv");
            message.innerHTML = "***Congrats you Won $" + (2 * parseInt(betamount)) + "***";
        }
        var lossAmount = 0;
        var noOfTrucks = userActions.trucksBetAmountArray.length;
        for (var i = 0; i < noOfTrucks; i++) {
            if (i != indexofTruck) {
                lossAmount += parseInt(userActions.trucksBetAmountArray[i]["betAmountOnTruckNumber"]);
            }
        }
        var currentfunds = document.getElementById("initialFund");
